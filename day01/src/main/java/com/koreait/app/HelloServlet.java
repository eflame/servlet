...
// @WebSerblet 어노테이션은 우리가 지정한 path(경로)로 요청이 들어왔을 때
// 이 서블릿이 실행되도록 설정해준다.
@WebServlet("/hello")
public class HelloServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
//	이 서블릿 클래스와 연결된 경로로 요청이 들어왔을때
//	요청 방식이 get | post 둘 중 하나가 들어오게된다.
//	get방식으로 요청이 들어오면 doGet() 메서드가 실행된다.
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
//		doGet 메서드의 파라미터를 확인해보면 request(요청), response(응답)이 선언되어 있다.
//		자바에서는 요청과 응답을 객체로 다루며, 사용자가 보낸 요청에서 필요한 정보를 커낼 때
...
//		charset=utf-8 -> 인코딩 타입이 utf-8인 것을 의미한다. (utf-8로 설정해야 한글이 깨지지 않는다.)
